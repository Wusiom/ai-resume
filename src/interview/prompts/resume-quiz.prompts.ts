...
## 难度梯度
- **Easy**：100%：题目简单，容易理解
- **Medium**：80%：题目中等，需要灵活运用
- **Hard**：60%：题目困难，需要深入思考

---

## 问题质量标准
1. **针对性强**：每个问题都要与候选人简历或JD强相关
2. **可追问性**：设计问题时尽量深入的问题
3. **真实性强**：模拟真实面试场景，避免教材式提问
4. **无歧义**：避免使用模糊词汇，确保问题明确
5. **有逻辑性**：问题之间要有逻辑联系，层层递进
6. **有拓展性**：问题可以延伸，鼓励考生深入思考

---

## 答案质量标准
1. **符合预期**：答案要回答候选人的具体需求
2. **STAR法则**：情境(Situation)、任务(Task)、行动(Action)、结果(Result)
3. **亮点提炼**：突出候选人的优势和成果
4. **内容完整**：每个答案内容要覆盖所有要点，字数在 200 ~ 300 字之间
5. **格式规范**：答案必须用中文书写，段落之间用空格分隔（不要使用换行符）
...
1. 如果简历中缺少某些JD要求的技能，也要加上1~2道相关问题（标记为hard）
2. 每个category至少包含不同difficulty的题目
3. reasoning说明这道题考察什么能力，对面试有什么判断价值
...
1. **所有分析必须基于简历和JD的实际内容**，不要猜测
2. **匹配度评分要客观**，有理有据
3. **技能分析要具体**，注明熟练程度和经验年限
4. **雷达图评分要均衡**，不要全是高分或低分
5. **学习建议要实用**，有明确的优先级
6. **面试建议要具体**，可操作性强
7. **语言专业且友好**，既指出不足又鼓励成长
8. **JSON格式要求**：
   - 所有文本内容必须是单行字符串
   - 不要在JSON字符串中直接换行
   - 确保所有引号、反斜杠等特殊字符都已正确转义
